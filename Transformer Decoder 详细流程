...
2. 添加位置编码（以正弦函数为例）：
 $$
 P(pos, 2i) = \sin(pos/10000^{2i/d_{\text{model}}}) \\
 P(pos, 2i+1) = \cos(pos/10000^{2i/d_{\text{model}}})
 $$
3. 最终输入矩阵：
 $$
...
- 计算 Q/K/V 矩阵：
$$
Q = T \cdot W_Q^d, \quad K = T \cdot W_K^d, \quad V = T \cdot W_V^d
$$
...
### 注意力计算
$$
\text{Attention}(Q, K, V) = \text{Softmax}\left( \frac{QK^T}{\sqrt{d_k}} + M \right) \cdot V
$$
其中 $d_k = d_{\text{model}}/h$
...
### 残差连接与层归一化
$$
T_{\text{new}} = \text{LayerNorm}( \text{Attention}(Q, K, V) + T )
$$
*多头情形* $ T_{\text{new}} = \text{LayerNorm}( \text{MultiHead}(Q, K, V) + T ) $ 
...
- 计算 Q/K/V：
$$
Q_{\text{cross}} = T \cdot W_Q^{\text{cross}}, \quad K_{\text{cross}} = Z \cdot W_K^{\text{cross}}, \quad V_{\text{cross}} = Z \cdot W_V^{\text{cross}}
$$
其中 $Z \in \mathbb{R}^{(m \times d_{\text{model}})}$ 为 Encoder 输出

### 注意力计算与归一化
$$
\text{CrossAttention} = \text{Softmax}\left( \frac{Q_{\text{cross}}K_{\text{cross}}^T}{\sqrt{d_k}} \right) \cdot V_{\text{cross}} \\
...
% 输出映射与预测
$$
\text{Logits} = T_{\text{final}} \cdot W_{\text{vocab}}, \quad W_{\text{vocab}} \in \mathbb{R}^{d_{\text{model}} \times V}
$$
